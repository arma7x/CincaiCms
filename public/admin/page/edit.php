<?php 
	...
	if (isset($_GET['index'])) {
		$file = $pages_metadata->seekMetadata($_GET['index']);
		if(count($file) === 0) {
			...
			$editor->getContent(__dir__.'/../../../application/pages');
		}
	}
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		if ($editor->removeFile(__dir__.'/../../../application/pages')) {
			...
			if ($editor->storeFile(__dir__.'/../../../application/pages')) {
				...
				$_SESSION['flash']['warning'] = 'Fail edit page `'.$_POST['title'].'`';
				header('Location: /admin/page');
				die;
			}
		} else {
			$_SESSION['flash']['success'] = 'Fail edit page `'.$_POST['title'].'`';
			header('Location: /admin/page');
			die;
		}
	}
...
<html lang="en">
	<head>
		...
		<title>Admin Panel - Edit Page[<?php echo $editor->title; ?>]</title>
	...
	<body>
		...
		<main role="main" class="container">
			...
			<form id="form-login" class="add-page" action="/admin/page/edit.php?index=<?php echo $_GET['index'] ?>" method="post">
				...
				<button id="login" class="btn btn-primary btn-sm" type="submit">Edit Page</button>
			</form>
		</main>
